(function (global) {
    ...
    function render(ctx, selected_value, field, revision) {
        if (prev_revision < 0) {
            canv_cache.canv_board = drawBoard();
            canv_cache.canv_pieaces = drawPieceALL(field);
            canv_cache.canv_effect = drawEffect(selected_value);
    ...
    function drawPieceALL(field) {
        ...
        for (var x = 0; x < COL; x++) {
            for (var y = 0; y < COL; y++) {
                if (field[y * COL + x] != 0) {
                    drawPiece(ctx, x * CELL_SIZE, y * CELL_SIZE, field[y * COL + x]);
                }
            }
        }
    ...
    function drawEffect(selected_value) {
        ...
        var x = (selected_value % COL | 0) * CELL_SIZE;
        var y = (selected_value / COL | 0) * CELL_SIZE;
